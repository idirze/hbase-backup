...
import org.apache.hadoop.hbase.backup.hbase1_2_1.*;
import org.apache.hadoop.hbase.backup.hbase1_2_1.BackupInfo.BackupState;
import org.apache.hadoop.hbase.backup.hbase1_2_1.impl.BackupManifest.BackupImage;
import org.apache.hadoop.hbase.backup.hbase1_2_1.regionserver.LogRollRegionServerProcedureManager;
...
public class BackupManager implements Closeable {
    ...
    public static void decorateMasterConfiguration(Configuration conf) {
        ...
        String classes = conf.get(ProcedureManagerHost.MASTER_PROCEUDRE_CONF_KEY);
        String masterProcedureClass = LogRollMasterProcedureManager.class.getName();
        if (classes == null) {
            conf.set(ProcedureManagerHost.MASTER_PROCEUDRE_CONF_KEY, masterProcedureClass);
        } else if (!classes.contains(masterProcedureClass)) {
            conf.set(ProcedureManagerHost.MASTER_PROCEUDRE_CONF_KEY, classes + "," + masterProcedureClass);
        }
    ...
    public ArrayList<BackupImage> getAncestors(BackupInfo backupInfo) throws IOException,
        ...
        for (BackupInfo backup : allHistoryList) {
            ...
            if (backup.getType().equals(BackupType.FULL)) {
                ...
                // then no need to add
                if (!org.apache.hadoop.hbase.backup.hbase1_2_1.impl.BackupManifest.canCoverImage(ancestors, image)) {
                    ancestors.add(image);
                }
            } else {
                ...
                if (org.apache.hadoop.hbase.backup.hbase1_2_1.impl.BackupManifest.canCoverImage(ancestors, image)) {
                    ...
                    org.apache.hadoop.hbase.backup.hbase1_2_1.impl.BackupManifest lastIncrImgManifest = new BackupManifest(conf, logBackupPath);
