...
import org.apache.hadoop.hbase.backup.hbase1_2_1.RestoreRequest;
...
public final class BackupUtils {
    ...
    public static void copyTableRegionInfo(Connection conn, BackupInfo backupInfo, Configuration conf)
        ...
        for (TableName table : backupInfo.getTables()) {
            ...
            List<HRegionInfo> regions = MetaTableAccessor.getTableRegions(null, conn, table);
    ...
    public static void writeRegioninfoOnFilesystem(final Configuration conf, final FileSystem fs,
        ...
        final byte[] content = HRegionInfo.toDelimitedByteArray(regionInfo);
    ...
    public static String parseHostNameFromLogFile(Path p) {
        try {
            if (AbstractFSWALProviderUtils.isArchivedLogFile(p)) {
                return BackupUtils.parseHostFromOldLog(p);
            } else {
                ServerName sname = AbstractFSWALProviderUtils.getServerNameFromWALDirectoryName(p);
                if (sname != null) {
                    return sname.getHostAndPort();
                } else {
    ...
    public static List<String> getWALFilesOlderThan(final Configuration c,
        ...
        PathFilter filter = p -> {
            try {
                if (AbstractFSWALProviderUtils.isMetaFile(p)) {
                    return false;
                }
    ...
    public static Path getBulkOutputDir(String tableName, Configuration conf, boolean deleteOnExit)
        ...
        String tmp = conf.get("hbase.fs.tmp.dir",
    ...
    public static LoadIncrementalHFiles createLoader(Configuration config) throws IOException {
        ...
        conf.set(LoadIncrementalHFiles.SILENCE_CONF_KEY, "yes");
