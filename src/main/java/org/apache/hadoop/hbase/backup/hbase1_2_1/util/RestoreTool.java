...
import org.apache.hadoop.hbase.backup.hbase1_2_1.RestoreJob;
...
public class RestoreTool {
    ...
    public static final Log LOG = LogFactory.getLog(RestoreTool.class);
    ...
    public void incrementalRestoreTable(Connection conn, Path tableBackupPath, Path[] logDirs,
        ...
        try (Admin admin = conn.getAdmin()) {
            ...
            for (int i = 0; i < tableNames.length; i++) {
                ...
                HTableDescriptor tableDesc = new HTableDescriptor(newTableDescriptor);
                ...
                for (HColumnDescriptor family : existingFamilies) {
                    if (!families.contains(family)) {
                        tableDesc.removeFamily(family.getName());
                        schemaChangeNeeded = true;
                    }
                }
                if (schemaChangeNeeded) {
                    modifyTableSync(conn, tableDesc);
     ...
     */
    HTableDescriptor getTableDesc(TableName tableName) throws IOException {
        Path tableInfoPath = this.getTableInfoPath(tableName);
        HBaseProtos.SnapshotDescription desc = SnapshotDescriptionUtils.readSnapshotInfo(fs, tableInfoPath);
    ...
    private void checkAndCreateTable(Connection conn, Path tableBackupPath, TableName tableName,
        ...
        try (Admin admin = conn.getAdmin()) {
            ...
            if (createNew) {
                SafeRestoreTool.createNamespaceIfNotExists(conn, targetTableName.getNamespaceAsString());
