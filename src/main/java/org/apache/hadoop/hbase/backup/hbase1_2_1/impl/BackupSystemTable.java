 ...
 */
package org.apache.hadoop.hbase.backup.hbase1_2_1.impl;
...
import org.apache.hadoop.hbase.backup.hbase1_2_1.BackupInfo.BackupState;
import org.apache.hadoop.hbase.backup.hbase1_2_1.BackupRestoreConstants;
import org.apache.hadoop.hbase.backup.hbase1_2_1.BackupType;
import org.apache.hadoop.hbase.backup.hbase1_2_1.protobuf.generated.BackupProtos;
...
public final class BackupSystemTable implements Closeable {
    ...
    private void waitForSystemTable(Admin admin, TableName tableName) throws IOException {
        ...
        LOG.debug("Backup table " + tableName
                + " is not present and available, waiting for it to become so");
        ...
        try (Table table = connection.getTable(bulkLoadTableName);
            ...
            while ((res = scanner.next()) != null) {
                ...
                for (Cell cell : res.listCells()) {
                    if (CellUtil.matchingQualifier(cell, BackupSystemTable.PATH_COL, 0,
                            BackupSystemTable.PATH_COL.length)) {
    ...
    public Map<byte[], List<Path>>[] readBulkLoadedFiles(String backupId, List<TableName> sTableList)
        ...
        try (Table table = connection.getTable(bulkLoadTableName);
            ...
            while ((res = scanner.next()) != null) {
                ...
                String path = null;
                for (Cell cell : res.listCells()) {
                    if (CellUtil.matchingQualifier(cell, BackupSystemTable.TBL_COL, 0,
                            BackupSystemTable.TBL_COL.length)) {
                        tbl = TableName.valueOf(CellUtil.cloneValue(cell));
                    } else if (CellUtil.matchingQualifier(cell, BackupSystemTable.FAM_COL, 0,
                            BackupSystemTable.FAM_COL.length)) {
                        fam = CellUtil.cloneValue(cell);
                    } else if (CellUtil.matchingQualifier(cell, BackupSystemTable.PATH_COL, 0,
                            BackupSystemTable.PATH_COL.length)) {
    ...
    public void writePathsPostBulkLoad(TableName tabName, byte[] region,
        ...
        try (Table table = connection.getTable(bulkLoadTableName)) {
            ...
            table.put(puts);
            LOG.info("written " + puts.size() + " rows for bulk load of " + tabName);
        }
    }
            ...
            try (Table table = connection.getTable(bulkLoadTableName);
                ...
                while ((res = scanner.next()) != null) {
                    ...
                    for (Cell cell : res.listCells()) {
                        ...
                        if (CellUtil.matchingQualifier(cell, BackupSystemTable.FAM_COL, 0,
                                BackupSystemTable.FAM_COL.length)) {
                            fam = Bytes.toString(CellUtil.cloneValue(cell));
                        } else if (CellUtil.matchingQualifier(cell, BackupSystemTable.PATH_COL, 0,
                                BackupSystemTable.PATH_COL.length)) {
                            path = Bytes.toString(CellUtil.cloneValue(cell));
                        } else if (CellUtil.matchingQualifier(cell, BackupSystemTable.STATE_COL, 0,
                                BackupSystemTable.STATE_COL.length)) {
    ...
    private BackupProtos.TableServerTimestamp toTableServerTimestampProto(TableName table,
       ...
       /* tstBuilder
                .setTableName(org.apache.hadoop.hbase.protobuf.ProtobufUtil.toProtoTableName(table));
*/
        tstBuilder
                .setTableName(org.apache.hadoop.hbase.protobuf.ProtobufUtil.toProtoTableName(table));
    ...
    fromTableServerTimestampProto(BackupProtos.TableServerTimestamp proto) {
        ...
        for (BackupProtos.ServerTimestamp st : list) {
            ServerName sn =
                    org.apache.hadoop.hbase.protobuf.ProtobufUtil.toServerName(st.getServerName());
    ...
    public static HTableDescriptor getSystemTableForBulkLoadedDataDescriptor(Configuration conf) {
        HTableDescriptor tableDesc = new HTableDescriptor(getTableNameForBulkLoadedData(conf));
        HColumnDescriptor colSessionsDesc = new HColumnDescriptor(SESSIONS_FAMILY);
