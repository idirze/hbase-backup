 ...
 */
package org.apache.hadoop.hbase.backup.hbase1_2_1.third.tool;
...
public class LoadIncrementalHFiles extends Configured implements Tool {
    ...
    protected void bulkLoadPhase(Table table, Connection conn, ExecutorService pool,
        ...
        for (Entry<ByteBuffer, ? extends Collection<LoadQueueItem>> e : regionGroups.asMap()
            ...
            final Callable<List<LoadIncrementalHFiles.LoadQueueItem>> call =
                    new Callable<List<LoadIncrementalHFiles.LoadQueueItem>>() {
                @Override
                public List<LoadIncrementalHFiles.LoadQueueItem> call() throws Exception {
                    List<LoadIncrementalHFiles.LoadQueueItem> toRetry =
    ...
    private boolean checkHFilesCountPerRegionPerFamily(
        ...
        for (Entry<ByteBuffer, Collection<LoadQueueItem>> e : regionGroups.asMap().entrySet()) {
    ...
    private List<LoadQueueItem> splitStoreFile(final LoadQueueItem item, final Table table,
        ...
        HColumnDescriptor familyDesc = table.getTableDescriptor().getFamily(family);
    ...
    protected List<LoadQueueItem> tryAtomicRegionLoad(final Connection conn,
        ...
        for (LoadIncrementalHFiles.LoadQueueItem lqi : lqis) {
        ...
        try {
            List<LoadIncrementalHFiles.LoadQueueItem> toRetry = new ArrayList<>();
    ...
    private void createTable(TableName tableName, String dirPath, Admin admin) throws IOException {
        ...
        visitBulkHFiles(fs, hfofDir, new LoadIncrementalHFiles.BulkHFileVisitor<HColumnDescriptor>() {
            @Override
    ...
    private void validateFamiliesInHFiles(Table table, Deque<LoadQueueItem> queue, boolean silence)
        ...
        Iterator<LoadIncrementalHFiles.LoadQueueItem> queueIter = queue.iterator();
        while (queueIter.hasNext()) {
            LoadIncrementalHFiles.LoadQueueItem lqi = queueIter.next();
    ...
    public static byte[][] inferBoundaries(SortedMap<byte[], Integer> bdryMap) {
        ...
        for (Entry<byte[], Integer> item : bdryMap.entrySet()) {
            if (runningValue == 0) {
