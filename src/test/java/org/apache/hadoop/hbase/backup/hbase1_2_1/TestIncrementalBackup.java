...
import org.apache.hadoop.hbase.backup.hbase1_2_1.impl.BackupAdminImpl;
import org.apache.hadoop.hbase.backup.hbase1_2_1.rule.HBaseClassTestRule;
...
public class TestIncrementalBackup extends TestBackupBase {
    ...
    public void TestIncBackupRestore() throws Exception {
        ...
        HBaseTestingUtility.modifyTableSync(TEST_UTIL.getHBaseAdmin(), table1Desc);
        ...
        try (Connection conn = ConnectionFactory.createConnection(conf1)) {
            ...
            Assert.assertEquals(new HBaseTestingUtility().countRows(t1),
            ...
            Assert.assertEquals(NB_ROWS_IN_BATCH + 5, new HBaseTestingUtility().countRows(t2));
            t2.close();
            ...
            Assert.assertEquals(new HBaseTestingUtility().countRows(hTable), NB_ROWS_IN_BATCH + NB_ROWS_FAM3);
            hTable.close();

            hTable = (HTable) conn.getTable(table2_restore);
            Assert.assertEquals(NB_ROWS_IN_BATCH, new HBaseTestingUtility().countRows(hTable));
            hTable.close();
            ...
            hTable = (HTable) conn.getTable(table2_restore);
            Assert.assertEquals(NB_ROWS_IN_BATCH + 5, new HBaseTestingUtility().countRows(hTable));
            hTable.close();
            admin.close();
        }
    }
